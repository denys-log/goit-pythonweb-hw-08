...
async def read_contacts(
    ...
    contact_service = ContactsService(db)
    contacts = await contact_service.get_contacts(name, email, skip, limit)
    return contacts


@router.get("/{contact_id}", response_model=ContactResponse)
async def react_contact(contact_id: int, db: AsyncSession = Depends(get_db)):
    contact_service = ContactsService(db)
...
async def update_contact(
    ...
    contact_service = ContactsService(db)
    tag = await contact_service.update_contact(contact_id, body)
    if tag is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Contact not found"
        )
...
async def remote_contact(contact_id: int, db: AsyncSession = Depends(get_db)):
    contact_service = ContactsService(db)
